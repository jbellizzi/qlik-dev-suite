...
import { concatMap, map, switchMap, tap, withLatestFrom, mergeMap } from "rxjs/operators"
	...
	new Observable(observer =>
		source
			.pipe(
				...
				switchMap(([_, appSheetsObj]) => from(appSheetsObj.getLayout())),
				map(layout =>
				...
				concatMap(sheet =>
					from(app.getObject(sheet.sheetId).then(obj => ({ sheetObj: obj, objects: sheet.objects }))).pipe(
						...
						switchMap(({ sheetObj, newProps }) => {
							console.log(newProps)
							return from(sheetObj.setFullPropertyTree(newProps))
						})
					)
				)
